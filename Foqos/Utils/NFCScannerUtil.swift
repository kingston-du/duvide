...
class NFCScannerUtil: NSObject {
  ...
  func scan(profileName: String, verb: String = "enter") {
    ...
    let target = profileName.isEmpty ? "" : " \(profileName)"
    nfcSession?.alertMessage = "Scan NFC to \(verb)\(target)"
    nfcSession?.begin()
  }
...
extension NFCScannerUtil: NFCTagReaderSessionDelegate {
  ...
  func tagReaderSession(_ session: NFCTagReaderSession, didDetect tags: [NFCTag]) {
    ...
    case .iso15693(let tag):
      identifier = tag.identifier.hexEncodedString()
    ...
    handleTagData(id: identifier, url: nil, session: session)
  }
